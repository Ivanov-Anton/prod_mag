...
ActiveAdmin.register_page 'department_with_max_min_value', label: 'asd' do
  ...
  content title: 'Отделы с максимальной и минимальной прибылью' do
    ...
    if min_department.present? && max_department.present?
      text_of_min = "Отдел с минимальной прибылью: #{min_department.name}"
      text_of_min << ", прибыль за все время состевляет всего: #{min_department.total_price} грн"
      h3 text_of_min

      text_of_max = "Отдел с максимальной прибылью: #{max_department.name}"
      text_of_max << ", рибыль за все время состевляет всего: #{max_department.total_price} грн"
      h3 text_of_max
    end
  end
end
